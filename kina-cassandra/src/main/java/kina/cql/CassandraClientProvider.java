...
class CassandraClientProvider {
    ...
    static Pair<Session, String> trySessionForLocation(String location, CassandraKinaConfig conf, Boolean balanced) {
        try {
    ...
    static Pair<Session, String> getSession(String location, CassandraKinaConfig conf, Boolean balanced) {
        ...
        synchronized (CLIENTS_CACHE) {
            ...
            try {
                ...
                throw new IOException("Failed to create authenticated client to {" + location + "}:{" + port +
                        "}", e);
            }
        }
