...
import kina.cql.DeepRecordReader;
...
import kina.functions.DeepType2TupleFunction;
import kina.partition.impl.DeepPartition;
...
public abstract class CassandraRDD<T> extends RDD<T> {
    ...
    public static <W, T extends IDeepType> void cql3SaveRDDToCassandra(RDD<W> rdd, CassandraKinaConfig<W> writeConfig) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <W, T extends IDeepType> void saveRDDToCassandra(RDD<W> rdd, CassandraKinaConfig<W> writeConfig) {
        if (IDeepType.class.isAssignableFrom(writeConfig.getEntityClass())) {
            CassandraKinaConfig<T> c = (CassandraKinaConfig<T>) writeConfig;
            RDD<T> r = (RDD<T>) rdd;
